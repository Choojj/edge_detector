...
path = "C:/Users/HSWB/Desktop/edge_detector/data/absdiff_03.png"

img_color = cv.imread(path, cv.IMREAD_COLOR)

img_gray = cv.cvtColor(img_color, cv.COLOR_BGR2GRAY)

ret,img_binary = cv.threshold(img_gray, 10, 255, cv.THRESH_OTSU)
...
for cnt in contours:
    ...
    cv.line(img_color, tuple(approx[0][0]), tuple(approx[size-1][0]), (0, 255, 0), 2)

    for k in range(size-1):
        cv.line(img_color, tuple(approx[k][0]), tuple(approx[k+1][0]), (0, 255, 0), 2)
